...
## 11. Next-chat prompt (supersedes §9) — includes the group-chat forwarding fix and the no-disconnect decision

Decision 2026-09-10: the stale integrations (Unified.to Teams MCP bound to Lua's tenant, Gmail, Google Calendar) are NOT to be disconnected — too risky. Instead the agent blocks those tools in code (governance `blockTools`), in its own version with a sandbox soak.

> You are continuing the Sucafina sample-management build in `/Users/devashishthapliyal/Documents/work/Lua/Sucafina`. Read `docs/HANDOVER-2026-09-09-phases3-5.md` in full first — §0–§8 is the state at commit 10ee2e0 (round 6 + Phases 3–5 all committed on `main`, NOTHING pushed or deployed, prod agent is v51), §10 is Harriet's answers and the pending-dispatch sheet findings. Then work in this order, TDD, one commit per task, and stop only when you need my standalone yes for a promote or the purge.
>
> **1. The forwarding / group-chat problem (the Beyers incident, round 6).** Ivo @mentions the bot in a Teams group chat that includes Tommie and says "ask Tommie for the Beyers address". Today (v51) the bot answers with Lua's own workspace channels (it consults a stale Unified.to Teams MCP) and says Tommie is unreachable. We are NOT disconnecting any integration. Do this instead: (a) add a governance guard on the `LuaAgent` in `src/index.ts` — `governance: { mode: 'sdk', rules: { blockTools: ['prepare_share', <every tool name exposed by the microsoftteamsbot / Gmail / Google Calendar MCP connections — enumerate them from `lua integrations` and the platform logs, not from memory>] } }` — in its OWN agent version, sandbox-soaked first (a July push once killed all tool execution; check the pinned lua-cli SDK types for `GovernanceConfig` before relying on it); keep the persona's OUR TOOLS ONLY rule; (b) make `request_missing_details` (src/skills/tools/RequestMissingDetailsTool.ts) group-aware: when the current conversation is a Teams group chat (not a 1:1) and the named colleague is a member or cannot be DM'd because they are cold, post the ask INTO THAT SAME CONVERSATION addressed to them by name via `Channels.send({ to: { conversationId } })` (lua-cli ≥ 3.26 supports group posting; the bot cannot attribute speakers in groups, so never rely on who said what), still record the ask on the client, and still send the email fallback with the QC mailbox + logger copied; return honest `{ delivered, via: 'group' | 'teams' | 'email', to }`; (c) verify the email leg end-to-end on sandbox (the inbound `ping@heymail.ai` channel does not read plain replies, so the email must say reply-all or use the dashboard link) and that Tommie is on the roster with his email; (d) persona/skill line: "in a group chat, turn to the named person in that chat; never say you sent something a tool did not confirm". Harness-test with an injected deliver. Version plan: v52 = current tree; v53 details-chaser; v54 tracking-sweep; v55 pss-schedule; v56 = the governance guard + group-aware tool (its own version because the guard is the risky part).
>
> **2. Critical importer bug:** `api/src/lib/pss-import.ts` CSV path lets SheetJS parse ambiguous dd/mm dates US-first (`01/12/2026` → 12 Jan). Make CSV cells strings before `parseShipmentDate` (`XLSX.read(text, { type: 'string', raw: true })` or a plain CSV parse); test `01/12/2026 → 2026-12-01` via csv; check xlsx date-typed cells too.
>
> **3. Apply Harriet's answers (handover §10) before v55 or any SOL import:** SSKE refs are CONTRACT-DERIVED — `SSKE-<contract digits>` + option letter(s), replacements take the next unused letters (A–C rejected → D–F); stop using `issueRef('pss')` for contract PSS (our counter at 108000 collides with real SSKE-108575/108577/108792); a PSS request = N lettered options × grams (client-specific: Nespresso 1 kg, Zoegas 600 g, JDE 300 g, CK 500 g — default from the client's last PSS, never a fixed 1 kg), `pss_expected` free of `containers`, add `po_ref` on contracts and an option letter on sample rows, rename "Container" to "Option" in UI/agent; second rejection → contract status `pss_replacement_rejected` AND draw again with the next letters (keep drawing per rejection; the flag clears on approval); reminders (`pss_due_soon`, `pss_overdue`) go to QC ONLY (move them to `QC_EVENTS` in `src/jobs/status-notifier.job.ts`); deleted sample refs are reusable when the deleted ref was the latest number for its prefix (decrement the SL/TYPE counter on soft-delete in that case); importer synonyms gain "quantity per sample" parsed as `options × grams`; the client matcher must tolerate "Zoegas / Nestlé Sverige", "Nestlé España (Japan destination)", "Marc Bang on behalf of CK CORPORATION"; adopt Harriet's status vocabulary in wording ("Pending PSS dispatch", "PSS dispatched", "Pending PSS results", "Sample approved", "Replacement PSS requested after rejection", "Pending Replacement Results", "PSS replacement rejected"). Migration 021 for the schema bits, idempotent, appended to `scripts/deploy-api.sh`. Questions 1/5/7/8/12/13 are still with Gloria and Ivo — build on the bracketed defaults in §6 and keep them one-line changes.
>
> **4. Review what was never reviewed:** one reviewer over `.superpowers/sdd/you-are-continuing-the-zazzy-candy/review-31293c3..c6df43e.diff` (importer, pss-schedule skill + notifier wording, dashboard Contracts pages, docs) against the Verification table in `/Users/devashishthapliyal/.claude/plans/you-are-continuing-the-zazzy-candy.md`, then ONE fix wave for Critical/Important; triage the 32 deferred minors in `.superpowers/sdd/you-are-continuing-the-zazzy-candy/progress.md` and fix the stale "simulated tracking" wording in ARCHITECTURE.md, README.md, DEMO.md.
>
> **5. Hand me the deploy sequence** (handover §3 + the purge list §4): one API deploy carrying 016–021 → RC7 roster fix → agent v52 → `git push origin main` → v53 → v54 → v55 → v56, each with `lua version diff`; I run every command; promote and the purge only on my standalone yes; DHL keys via Daniel Chege, FedEx via Brillian Cherono.
>
> Conventions: vitest + supertest in `api/test` (RED before GREEN; never run two API suites concurrently — they share `sucafina_test`); idempotent migrations; every alert rides `notifications_outbox`; agent primitives one per version; `lua compile` does not typecheck — run the focused tsc; never commit CHOTU-CONTEXT-sucafina.md, popover-snap.md, sl-8008-drawer.png, docs/teams-agent-connect-instructions.md; no Co-Authored-By trailer; never "prototype/simulated" in agent prose or UI; the Bash hook may reject `$` — use a .sh file.
