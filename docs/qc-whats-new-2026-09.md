...
- **Lettered options, the way you write them.** A contract owes N PSS **options** — A, B, C… — of the client's usual size (Nespresso 1 kg, Zoegas 600 g, JDE 300 g, CK 500 g; the bot takes it from the contract or the client's last PSS, never a fixed 1 kg). The refs are the contract's: **SSKE-104929A, SSKE-104929B…**, and the contract keeps the client's PO ref. The new **Contracts** page shows each option, its stage in your words (Pending PSS dispatch → PSS dispatched → Pending PSS results → Sample approved), and "1 of 3 approved" at a glance. Ask the bot "what PSS are due?" or "where are we on SSKE-104929?" and you get the same picture in chat.
- **Reminders.** 14 days, 7 days and on the due date, then once a week for as long as a contract stays overdue. They go to the Quality team only, and they stop by themselves once every option is approved.
- **A rejection is handled for you.** When a client rejects an option, the replacement is raised automatically with the **next letter** (A–C rejected → D–F) — record the rejection as usual and the bot tells you the new ref. If the client rejects the replacement too, the contract is flagged **PSS replacement rejected** and you and the account manager are told; the next option is still drawn, and the flag clears when one is approved.
- **The schedule comes from the SOL report.** Send the SOL export to the bot as **Excel or CSV** (a PDF cannot be read — export the same report instead). It shows you a preview first: how many contracts it would create or update, how many options it would raise (it reads "3x600grams" as three options of 600 g and keeps the PO ref), which columns it read, and anything it could not make sense of. Client names like "Zoegas / Nestlé Sverige" or "Marc Bang on behalf of CK Corporation" are matched to the book by themselves. Nothing is written until you say go, and re-sending the same schedule updates the contracts without raising a second set of samples.
- **On the dashboard:** the **Contracts** page lists what is due (overdue in red, the next week in amber) and each contract opens on its options, where a missing one can be drawn with one click. The Commercial book now shows the contract number as a link, the option letter and the PSS due date, and can be filtered to what is overdue. Deleting a sample frees its ref when it was the latest number, so the next request reuses it.
