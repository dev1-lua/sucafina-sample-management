...
export default class UpdateSampleStatusTool implements LuaTool {
  ...
  description =
    'Mark a sample as being prepared by the lab (status → preparing) — the sales trader is pinged automatically as it progresses. Pass tab + id if you already have them (from search_samples / find_open_samples), or just the ref (e.g. "SL-8007") and it resolves the row. Dispatches stay with record_dispatch; results stay with the results tools. Returns the updated row card fields.';
