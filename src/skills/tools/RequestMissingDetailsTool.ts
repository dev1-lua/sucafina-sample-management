...
export default class RequestMissingDetailsTool implements LuaTool {
  ...
  async execute(input: z.infer<typeof this.inputSchema>) {
    ...
    if (input.to_email) {
      ...
      const cands = matchTraderCandidates(input.to_name, traders);
      if (cands.length > 1) {
        throw new Error(`Several people on the roster match "${input.to_name}": ${cands.map((c) => c.name).join(', ')}. Ask which one (or their email) and retry.`);
      }
      ...
      if (!to) {
        ...
        reason = hit
          ...
          : `"${input.to_name}" is not on the roster — ask for their work email once, then retry with to_email. If nobody has it, call again with no to_name and I'll record the gap for the daily chase.`;
      }
    } else {
